	...
	private boolean checkMatch(CraftingInventory craftingInventory, int width, int height,
		...
		for (int i = 0; i < craftingInventory.getWidth(); ++i) {
			for (int j = 0; j < craftingInventory.getHeight(); ++j) {
				...
				ItemStack original = ItemCompressed
						.getOriginal(craftingInventory.getStackInSlot(i + j * craftingInventory.getWidth()));

				if (!ingredient.test(original)) {
					return false;
				}
			}
		}
	...
	public static class Serializer extends net.minecraftforge.registries.ForgeRegistryEntry<IRecipeSerializer<?>>
		...
		public ShapedRecipe read(ResourceLocation recipeId, JsonObject json) {
			...
			String[] astring = ReplacedShapedRecipe
					.shrink(ReplacedShapedRecipe.patternFromJson(JSONUtils.getJsonArray(json, "pattern")));
